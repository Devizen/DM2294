...
class Shoe : public Equipment
{
	Mesh* modelMesh;
public:
	Shoe() {};
	Shoe(Mesh* modelMesh) { this->modelMesh = modelMesh; };
	~Shoe() {};
...
namespace Create
{
	Shoe* theShoe(const std::string& _meshName, string ID, string attack, string defense, string speed, bool isEquipped);
};